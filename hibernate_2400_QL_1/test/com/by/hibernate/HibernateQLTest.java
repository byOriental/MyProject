...
public class HibernateQLTest {
	...
	public void testHQL_01() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("from Category");// 面向对象查询语言
	...
	public void testHQL_07() {
		...
		session.beginTransaction();
		Query q = session.createQuery("from Category c order by c.name desc");
		q.setMaxResults(3);// 分页显示，每页显示3条记录
		q.setFirstResult(2);// 从第二条开始显示
		List<Category> categories = (List<Category>) q.list();
		for (Category c : categories) {
			System.out.println(c.getId() + "-" + c.getName());
		}
	...
	public void testHQL_09() {
		Session session = sf.openSession();
		session.beginTransaction();
		Query q = session.createQuery("from Topic t where t.category.id = 1");// 显示category为1的topic信息
